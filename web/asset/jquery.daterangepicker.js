...
(function($)
{
  ...
  $.fn.dateRangePicker = function(opt)
  {
    ...
    function init_datepicker()
    {
      ...
      showMonth(opt.showPrevMonth?prevMonth(defaultTime):defaultTime,'month1');
      showMonth(opt.showPrevMonth?defaultTime:nextMonth(defaultTime),'month2');
          ...
          if (opt.customShortcuts && opt.customShortcuts.length > 0)
          {
            for(var i=0;i<opt.customShortcuts.length;i++)
            {
              ...
              if (sh.name == name)
              {
                ...
                {
                  movetodate = data[0];
                  showMonth(opt.showPrevMonth?prevMonth(movetodate):movetodate,'month1');
                  showMonth(opt.showPrevMonth?movetodate:nextMonth(movetodate),'month2');
                  showGap();
                }
    ...
    function setDateRange(date1,date2)
    {
      ...
      if (!valid)
      {
        showMonth(opt.showPrevMonth?prevMonth(opt.startDate):opt.startDate,'month1');
        showMonth(opt.showPrevMonth?opt.startDate:nextMonth(opt.startDate),'month2');
        showGap();
        return;
      }
      ...
      {
        date2 = opt.showPrevMonth?prevMonth(date1):nextMonth(date1);
        if(opt.showPrevMonth){
          var tmp = date1;
          date1 = date2;
          date2 = tmp;
          tnp = null;
    ...
    function showSelectedDays()
    {
      ...
      box.find('.day').each(function()
      {
        ...
        if (opt.time.enabled) {
          ...
          time = moment(moment(time).format('YYYY-MM-DD')).startOf('day').valueOf();
          start = moment(moment(start).format('YYYY-MM-DD')).startOf('day').valueOf();
          end = moment(moment(end).format('YYYY-MM-DD')).startOf('day').valueOf();
        }
        if (
    ...
    function closeDatePicker()
    {
      ...
      if(!$('.date-picker-wrapper') || !$('.date-picker-wrapper').is(':visible')){return;}
      $(box).slideUp(200,function()
      {
        $(self).data('date-picker-opened',false);
      });
      //$(document).unbind('.datepicker');
      if(isNaN(opt.start) || isNaN(opt.end)){
        return;
      }
